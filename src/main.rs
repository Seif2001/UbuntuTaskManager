...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        if app.should_render {
            ...
            let main_view_constraints = [Constraint::Min(0), Constraint::Length(3),  Constraint::Length(3)].as_ref();
            ...
            terminal.draw(|mut f| {
                render_processes_layout(&mut f, main_view_layout[0], &app);
                render_console_layout(&mut f, main_view_layout[1], &app);
                render_input_layout(&mut f, main_view_layout[2], &app);
            })?;
